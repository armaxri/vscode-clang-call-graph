...
suite("Database HppFile equality tests", () => {
    [DatabaseType.lowdb].forEach(async (testData) => {
        test(`Test equality for simple C++ header file on ${DatabaseType[testData]}`, async () => {
    ...
    [DatabaseType.lowdb].forEach(async (testData) => {
        test(`Test no equality for simple C++ header file on ${DatabaseType[testData]} based wrong file name`, async () => {
            const [database, referenceDatabase] =
                await prepareDatabaseEqualityTests(
                    __dirname,
                    "simple_func_decl_expected_db.json",
