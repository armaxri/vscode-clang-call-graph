...
suite("Database CppFile equality tests", () => {
    [DatabaseType.lowdb, DatabaseType.sqlite].forEach(async (testData) => {
        test(`Test equality for simple C++ file on ${DatabaseType[testData]} with one function declaration`, async () => {
    ...
    [DatabaseType.lowdb, DatabaseType.sqlite].forEach(async (testData) => {
        test(`Test equality for simple C++ file on ${DatabaseType[testData]} with one function implementation`, async () => {
            const [database, referenceDatabase] =
                await prepareDatabaseEqualityTests(
                    __dirname,
    ...
    [DatabaseType.lowdb, DatabaseType.sqlite].forEach(async (testData) => {
        test(`Test equality for simple C++ file on ${DatabaseType[testData]} with multiple function implementations`, async () => {
            const [database, referenceDatabase] =
                await prepareDatabaseEqualityTests(
                    __dirname,
    ...
    [DatabaseType.lowdb, DatabaseType.sqlite].forEach(async (testData) => {
        test(`Test not equality for simple C++ file on ${DatabaseType[testData]} with multiple function implementations (missing implementation)`, async () => {
            const [database, referenceDatabase] =
                await prepareDatabaseEqualityTests(
                    __dirname,
                    "multiple_simple_func_impl_expected_db.json",
    ...
    [DatabaseType.lowdb, DatabaseType.sqlite].forEach(async (testData) => {
        test(`Test no equality for simple C++ file on ${DatabaseType[testData]} based wrong file name with one function declaration`, async () => {
            const [database, referenceDatabase] =
                await prepareDatabaseEqualityTests(
                    __dirname,
                    "simple_func_decl_expected_db.json",
    ...
    [DatabaseType.lowdb, DatabaseType.sqlite].forEach(async (testData) => {
        test(`Test no equality for simple C++ file on ${DatabaseType[testData]} based wrong file name with one function impl`, async () => {
            const [database, referenceDatabase] =
                await prepareDatabaseEqualityTests(
                    __dirname,
                    "simple_func_impl_expected_db.json",
    ...
    [DatabaseType.lowdb, DatabaseType.sqlite].forEach(async (testData) => {
        test(`Test no equality for simple C++ file on ${DatabaseType[testData]} based wrong function name with one function declaration`, async () => {
            const [database, referenceDatabase] =
                await prepareDatabaseEqualityTests(
                    __dirname,
                    "simple_func_decl_expected_db.json",
    ...
    [DatabaseType.lowdb, DatabaseType.sqlite].forEach(async (testData) => {
        test(`Test no equality for simple C++ file on ${DatabaseType[testData]} based wrong function name with one function implementation`, async () => {
            const [database, referenceDatabase] =
                await prepareDatabaseEqualityTests(
                    __dirname,
                    "simple_func_impl_expected_db.json",
    ...
    [DatabaseType.lowdb, DatabaseType.sqlite].forEach(async (testData) => {
        test(`Test no equality for simple C++ file on ${DatabaseType[testData]} based wrong function location with one function declaration`, async () => {
            const [database, referenceDatabase] =
                await prepareDatabaseEqualityTests(
                    __dirname,
                    "simple_func_decl_expected_db.json",
    ...
    [DatabaseType.lowdb, DatabaseType.sqlite].forEach(async (testData) => {
        test(`Test no equality for simple C++ file on ${DatabaseType[testData]} based wrong function location with one function implementation`, async () => {
            const [database, referenceDatabase] =
                await prepareDatabaseEqualityTests(
                    __dirname,
                    "simple_func_impl_expected_db.json",
